...
import { ProfileTabType, TabConfig } from './ProfileBanner.types';

export const useProfileBanner = (onTabChange: (tab: ProfileTabType) => void) => {
  ...
  const handleTabClick = useCallback((key: ProfileTabType) => {
    onTabChange(key);
  }, [onTabChange]);
